...
(function setupPulsePrepQuestionBankFilters() {
  ...
  console.log(
    ...
    const oldQuestionBankFilters =
      section.querySelector("#pulsePrepQuestionBankFilters");

    if (oldQuestionBankFilters) {
      oldQuestionBankFilters.remove();
    }
    ...
    filterCard.innerHTML = `
      <div class="flex flex-col gap-4">
        ...
        <div>
          ...
          <p class="text-sm text-slate-500 mt-1">
            Search questions or filter by subject, topic, or difficulty.
          </p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row
          ...
          <div
            id="pulsePrepQuestionFilterCount"
            class="text-sm font-bold text-slate-600">
          </div>
    ...
    questionGroups.forEach(group => {
      ...
      cards.forEach(card => {
        ...
        if (difficulty) {
          difficultyValues.add(difficulty);
        }

        const questionText =
          card.querySelector("p.font-bold")
            ?.textContent
            ?.trim() || card.textContent || "";

        card.dataset.qbSubject =
          subject.toLowerCase();

        card.dataset.qbTopic =
          topic.toLowerCase();

        card.dataset.qbDifficulty =
          difficulty.toLowerCase();

        card.dataset.qbSearch =
          (
            questionText +
            " " +
            subject +
    ...
    function updateQuestionBankFilters() {
      ...
      questionCards.forEach(item => {
        ...
        const matchesSearch =
          !search ||
          card.dataset.qbSearch.includes(
            search
          );
      ...
      countDisplay.textContent =
        `Showing ${visibleCount} of ${originalTotal} questions`;
    ...
    console.log(
      `PulsePrep Question Bank filters ready: ${originalTotal} questions.`
    );
      return;
    }
